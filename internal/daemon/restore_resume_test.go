...
func TestBuildClaudeCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.want {
				t.Errorf("buildClaudeCommand(%v, %q) = %q, want %q",
...
func TestWriteClaudeScript_IncludesResumeFlag(t *testing.T) {
	...
	if !strings.Contains(contents, "claude --resume sess-xyz") {
		t.Errorf("expected script to contain 'claude --resume sess-xyz', got:\n%s", contents)
	}
}
