...
// match the task's active step AND the task_steps row must still be 'running'.
// There is no MCP-server-side notion of "the calling agent's task ID", so the
// client must pass task_id explicitly; misrouted updates are caught by the
// active-step check rather than by trusting the caller.
